...
public class FollowXLineSwerveRequestCommand extends Command {
	 ...
	 */
	public FollowXLineSwerveRequestCommand(Drive drive, Superstructure superstructure) {
	...
	public void initialize() {
		Distance driveX = drive.getPose().getMeasureX();
	...
	public void execute() {
		...
		drive.getDrivetrain().setControl(DriveConstants.PIDToPoseRequest.withVelocityX(desiredSpeeds.vxMetersPerSecond)
				.withVelocityY(desiredSpeeds.vyMetersPerSecond)
				.withRotationalRate(desiredSpeeds.omegaRadiansPerSecond));

		// if (driveDone()) {
		// 	Superstructure.setDriveReady(true);
		// 	ControlBoard.setRumble(true);
		// } else {
		// 	Superstructure.setDriveReady(false);
		// 	ControlBoard.setRumble(false);
		// }
	}
