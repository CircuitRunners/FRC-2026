...
public class MotorIOTalonFXSim extends MotorIOTalonFX {
	...
	private void updateSimStates() {
		main.getSimState()
				.setRotorVelocity(sim.mechanismToRotor(sim.getVelocity().times(getMotorInvertMultiplier())).in(Units.RotationsPerSecond));
		main.getSimState()
				.setRawRotorPosition(sim.mechanismToRotor(sim.getPosition().times(getMotorInvertMultiplier())).in(Units.Rotations));
